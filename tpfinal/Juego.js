class Juego {
  constructor() {
    this.estado = "Juego";
    const tamañoPersonajesEnemigos = window.innerWidth < 700 ? 50 : 100;
    this.personaje = new Personaje(width / 2, 150, tamañoPersonajesEnemigos);
    this.enemigos = [];
    ...
    this.fondoY = 0;
    this.fondo = loadImage("assets/imagenes/fondo.jpg");
    this.vidas = 10;
  ...
  dibujarBotones() {
    this.botonIzquierdo.dibujar();
    this.botonDerecho.dibujar();
  }
  ...
  mousePresionado() {
    if (this.botonIzquierdo.clicEnBoton(mouseX, mouseY)) {
      this.personaje.movimientoIzq = true;
    } else if (this.botonDerecho.clicEnBoton(mouseX, mouseY)) {
      this.personaje.movimientoDer = true;
    }
  }

  mouseLiberado() {
    this.personaje.movimientoIzq = false;
    this.personaje.movimientoDer = false;
  }
  ...
  iniciar() {
    image(this.fondo, 0, this.fondoY + 15, width, 5500);
